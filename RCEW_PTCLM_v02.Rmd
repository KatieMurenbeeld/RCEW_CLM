...
The Community Terrestrial Systems Model [CTSM](https://www.cesm.ucar.edu/models/cesm2/land/) (or [CLM](https://www.cesm.ucar.edu/models/clm/)) is the land model componentof the CESM. Often, one may see or hear CTSM refered to as CLM (Community Land Model). Technically, CLM is a specific component of the CTSM, but the terms are often used interchangably. CLM is used for examining the biophysical and biogeochemical processes of the Earth's terrestrial systems. CLM contains various submodel options such as different river routing schemes or dynamic vegetation components. I highly recommend the [overview](https://www.cesm.ucar.edu/models/clm/) as well as some of the powerpoints and [tutorials](https://www.cgd.ucar.edu/events/2019/ctsm/) from the [github page](https://github.com/ESCOMP/CTSM/wiki/Model-Tutorials) for a more in depth overview. Additionally, the techinal documentation for CLM version 5.0 can be found [here](https://escomp.github.io/ctsm-docs/versions/release-clm5.0/html/tech_note/index.html).
...
The **P**oin**T** **CLM** runs through the steps for creating single point domain and surface data files which are described in [Section 1.6.3.5 of the CLM5.0 User's Guide](https://escomp.github.io/ctsm-docs/versions/release-clm5.0/html/users_guide/running-single-points/running-single-point-configurations.html#example-creating-a-surface-dataset-for-a-single-point) and [Chapter 2 of the CLM4.5 User's Guide](https://www.cesm.ucar.edu/models/cesm1.2/clm/models/lnd/clm/doc/UsersGuide/c11242.html). PTCLM is a simpler way to create these files specifically for Ameriflux tower sites. The file PTCLMmkdata runs the tools to get datasets set up, and copies them to a location you can use, including the changes needed for a case to use the dataset with namelist and XML changes. There are a few steps you will need to take before you can run the PTCLMmkdata script. For more information on the PTCLMmkdata script see the README file here: 
...
There you will find three different text files. One for site data (PTCLMDATA_sitedata.txt), one for soils data (PTCLMDATA_soildata.txt), and one for plant functional type data (PTCLMDATA_pftdata.txt). 
...
Soil data from Reynold's Mountain East was found at the [Dataset for Soil properties of Reynolds Mountain East a subcatchment of Reynolds Creek, Idaho ](https://doi.org/10.18122/B29T3T). For this example, I averaged the sand and clay contents. Soil data for other sites at RCEW can be found at the  [hydroshare website](https://www.hydroshare.org/group/143). 

Add the following information to the **PTCLMDATA_soildata.txt** using your text editor:
...
Add the following information to the **PTCLMDATA_pftdata.txt** using your text editor:

US-Rwe,100,9,0,0,0,0,0,0,0,0

Plant data was inferred from the Ameriflux site description [here](https://ameriflux.lbl.gov/sites/siteinfo/US-Rwe). At Rwe, 100% of the plants are broadleaf evergreen shrubs (i.e. sage brush).
...
dt %>%
  kbl(caption = "Plant functional type (PFT) Index", 
      col.names = c("Long Name",
                    "PFT Index")) %>%
  kable_paper() %>% 
...
Modules needed for PTCLM. *module_name/module.version*: 

* **ncarenv**: sets up default NCAR environments (needed for capability to enter a qinteractive session)
* **intel/17.0.1 or intel/18.0.5**: loads intel compilers: C (icc), C++ (icpc), and Fortran (ifort)
...
After loading the modules, change directory to the PTCLM folder. From there execute the buildtools script.
...
Once all of the modules are loaded, the tools are built, and the PTCLMmkdata script is updated, open up a qinteractive session in order to complete the map generation and file creation process. The process can take several hours, so I suggest setting the walltime to 6 hours. More information about the PBS batch system and qinteractive sessions can be found [here](https://www2.cisl.ucar.edu/resources/computational-systems/cheyenne/running-jobs/submitting-jobs-pbs).

The -X argument is is to make sure tha X11 forwarding. The -A argument allows you to set your project number (in my case UBOI0006).

**Note on X11 forwarding**: when you log onto Cheyenne you will have logged on with either -X (Windows) or -Y (Mac) to allows a remotely started application to show up on a local machine, i.e. X11 forwarding. 

```{bash ptcml_qint, echo=TRUE, eval=FALSE}
qinteractive -X -A UBOI0006 -l walltime=06:00:00
```

Activate a python 2 compatible miniconda environment (optional). 
...
```
**Note:** Setting up miniconda environments is beyond the scope of this technical note. Sometimes there can be conflicts between modules on Cheyenne and modules in the minoconda environment. One useful thing to do if you are running into issues is to run *module purge* before loading the other modules, load python 2, and not worry about using a miniconda environment. 


After entering the qinteractive environment, you will need to set or reset the following environmental variables.
```{bash ptml_setenv, echo=TRUE, eval=FALSE}
## File path to inputdata
CSMDATA=/glade/p/cesm/cseg/inputdata

## File path to you cloned ctsm 
CTSMROOT=/glade/work/katiem/git/ctsm_rcew

## The site you want to create surface data files for
SITE=US-Rwe
```

Next, run the PTCLMsublist
```{bash ptclm_sublist, echo=TRUE, eval=FALSE}
./PTCLMsublist -l $SITE -d $CSMDATA -o --verbose --account=UBOI0006 --mach=cheyenne

## If you do not have miniconda, then make sure to module load python/2. When you run the PTCLM scripts make sure to have python in the command path
# python PTCLMsublist -l $SITE -d $CSMDATA -o --verbose --account=UBOI0006 --mach=cheyenne
```
...
Within this section we will work through the spinup and post spinup needed to run a simulation. The majority of these instructions are taken from the CTSM 2019 tutorial [practical 4](https://www.cgd.ucar.edu/events/2019/ctsm/files/practical4-wieder.pdf). All of the practicals are incredibly useful and can be found [here](https://www.cgd.ucar.edu/events/2019/ctsm/).
...
./create_newcase --compset 2000_DATM%GSWP3v1_CLM50%BGC_SICE_SOCN_SROF_SGLC_SWAV --res f09_g17 --case /glade/work/katiem/SITS_cases/Rwe1_CLM50spinup_001 --run-unsupported --project UBOI0006
```

In order to create a new case there are 3 required arguments: compset, res, and case. 

The **compset** argument sets the "component set". This specifies or sets all the model components used in the simulation. For example, this sets the river routing scheme or essentially turns of a model component by using a "stub". Many times land only simulations can use the stub sea-ice component. All the compsets can be found [here](https://www.cesm.ucar.edu/models/cesm2/config/compsets.html).
...
3. **Build the model** (sometimes after more changes to environment variables)
...
Once the new case is created, move into the newly created case directory. Within the case directory you will find the scripts to run the case setup (case.setup), build the model (case.build), and submit the case to the batch system (case.submit). 
...
After you've updated the above variables, you are now ready to setup the case.
...
Next, you will point to the domain files you created in Section 3. Notice the file paths set for ATM_DOMAIN_PATH and LND_DOMAIN_PATH. It is optional to copy and paste the surface data files created in section 3 to a different directory. If you do not, the ATM_ and LND_DOMAIN_PATH would be /glade/work/katiem/git/ctsm_rcew/tools/PTCLM/mydatafiles/1x1_US-Rwe
...
You can also check your scratch or scratch archive to check on the progress of the spinup case (i.e. the creation of output files in the case's scratch archive or the creation of files and logs in the run directory for the case)

```{bash sp_scratch, echo=TRUE, eval=FALSE}
cd /glade/scratch/$USER/archive/$CASE/lnd/hist
ls 
...
Congratulations! If all went well, you now have a "spunup" case (.nc file(s)) to work with. 
...
3. case.setup
4. Many more ./xmlchanges - changes to run from 1850-1901 - and namelist changes
5. Build the model
6. Submit case 
...
f09_g17 --case /glade/work/katiem/SITS_cases/Rwe1_CLM50postspin001 --run-unsupported --project UBOI0006
...
echo "mapalgo = 'nn','nn','nn','nn','nn'" >> user_nl_datm
echo "finidat = '/glade/scratch/katiem/archive/Rwe1_CLM50spinup_001/rest/0501-01-01-00000/Rwe1_CLM50spinup_001.clm2.r.0501-01-01-00000.nc'" >> user_nl_clm
echo "fsurdat = '/glade/work/katiem/SITS_data/surfdata_1x1pt_US-Rwe_hist_16pfts_Irrig_CMIP6_simyr2000_c200721.nc'" >> user_nl_clm
...
cd /glade/scratch/katiem/Rwe1_CLM50postspin_001/run
## remove the bin file
...
2. ./xmlchanges before running case.setup
3. case.setup --reset
4. ./xmlchanges and namelist changes
5. Build the case
...
## Creat the clone
./create_clone --case /glade/work/katiem/SITS_cases/Rwe1_CLM50_clone --clone /glade/work/katiem/SITS_cases/Rwe1_CLM50postspin001 --project UBOI0006
...
Changing any of the XML settings in env_mach_pes.xml requires ./case.setup –-reset. Since this is changing from the cloned case, we need to use the --reset argument.
...
./xmlchange RUN_REFCASE=Rwe1_CLM50postspin_001
...
The rpointer files are the restart pointer files which contain the restart filename written by each component of the model. These files will help the new case to read the correct files for restarting a simulation from a particular point. See more information about restarting a run [here](https://www.cesm.ucar.edu/models/cesm1.0/cesm/cesm_doc_1_0_4/x2094.html).

```{bash clone06, echo=TRUE, eval=FALSE}
## Copy over the rpointer files
cp -avr /glade/scratch/katiem/Rwe1_CLM50postspin_001/run/rpointer.* /glade/scratch/katiem/Rwe1_CLM50_clone/run/

cp -avr /glade/scratch/katiem/Rwe1_CLM50postspin_001/run/Rwe1_CLM50postspin_001* /glade/scratch/katiem/Rwe1_CLM50_clone/run/

## Check to see that they copied over correctly
cd /glade/scratch/katiem/Rwe1_CLM50_clone/run/
ls
```

#### 5.7 Copy over the archive output (history) files

Since this is a cloned case you will also need to write or copy over the output history files. If you were restarting a simulation from an exisiting (already run) case instead of a clone this step and the steps in 5.6 would not be necessary.
...
mkdir Rwe1_CLM50_clone

cp -avr /glade/scratch/katiem/archive/Rwe1_CLM50postspin_001/* /glade/scratch/katiem/archive/Rwe1_CLM50_clone/

## Check to make sure they copied over correctly
cd /glade/scratch/katiem/archive/Rwe1_CLM50_clone/lnd/hist
ls
```
...
Below is a list of useful links for a quick reference. Remember, google is your friend. I tend to start searches with NCAR, Cheyenne, CESM, CTSM or CLM followed by a copy-paste of whatever error (usually) or subject of interest. The [CESM discussion forum](https://bb.cgd.ucar.edu/cesm/) is also incredibly useful. You will have to make an account, and if you have a question make sure to follow the instructions regarding which information to include when starting a thread. 
...
CLM5.0 README [link](https://escomp.github.io/ctsm-docs/versions/release-clm5.0/html/users_guide/overview/introduction.html?highlight=resubmit#readme-file-describing-version)

CESM discussion forum [link](https://bb.cgd.ucar.edu/cesm/)
...
Cheyenne PBS batch system and qinteractive sessions information [link](https://www2.cisl.ucar.edu/resources/computational-systems/cheyenne/running-jobs/submitting-jobs-pbs)
