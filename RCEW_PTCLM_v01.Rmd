...
setenv USER katiem
setenv CTSMROOT /glade/work/${USER}/git/ctsm_rcew
```

And then run all of the same commands as in the script above but in the command line. Now, let's break this script down. 

The first step is to load the appropriate modules (i.e. packages and libraries). The module **ncl** (NCAR command language) is required to run some scripts. The Earth Systems Model Framework library (**esmf_libs/7.1.0r**) is needed to run some of the mkmap scripts. In order use **esmf_lib/7.1.0r** and ncl you will need to use intel version 17.0.1 or 18.0.5. **Note!** You can easily look up which versions of modules are required by using the module spider command.
